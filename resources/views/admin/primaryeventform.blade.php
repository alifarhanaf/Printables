...
@if (isset($primaryEevent))

    <form action="{{ route('submit.edited.category', $primaryEevent->id) }}" method="POST" enctype="multipart/form-data">
    @else
        <form action="{{ route('submitPrimaryEvent') }}" method="POST" enctype="multipart/form-data">
@endif
{{ csrf_field() }}
    ...
    <div class="row">
        <div class="col-md-9 ">
            <div class="az-content-label mg-b-5 ">
              @if(isset($primaryEevent))
              <p class="pd-t-10">Edit Your Primary Event</p>
              @else
              <p class="pd-t-10">Enter New Primary Event</p>
              @endif
...
<div class="row">
    <div class="col-md-9 mg-t-15">
        ...
        <div class="d-flex flex-column wd-md-100% pd-30 pd-sm-40 bg-gray-200">
            <label class="form-label mg-b-10 pd-l-5"><b>Enter Primary Event Name:</b></label>
            <div class="form-group">
                <input name="name" type="text" class="form-control"
                @if(isset($primaryEevent))
                placeholder="{{$primaryEevent->name}}"
                value="{{$primaryEevent->name}}"
                @else 
            ...
            <div class="form-group row row-sm  mg-b-10">
                <div class="col-lg">
                    ...
                    @if(isset($primaryEevent)) 
                    placeholder="{{$primaryEevent->description}}">{{$primaryEevent->description}}
                    @else 
        ...
        <div class="card bd-0">
            ...
            <div class="card-body bd bd-t-0">
                <div style="margin-left: 8%">
                    <div class="row"><input style="margin-top: 2%" type="radio" name="status" value="1" @if (isset($primaryEevent))


                        {{ $primaryEevent->enabled == 1 ? 'checked' : '' }}

                        @endif
                        checked
                        > <label style="margin-left: 3%">Enable</label><br>
                    </div>
                    <div class="row"><input style="margin-top: 2%" type="radio" name="status" value="0" @if (isset($primaryEevent))
                        {{ $primaryEevent->enabled == 0 ? 'checked' : '' }}
                        @endif
